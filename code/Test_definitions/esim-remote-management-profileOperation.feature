Feature: CAMARA eSIM Remote Management API v0.1.0-alpha.1 - Operation profileOperation
  ...
  Background: Common profileOperation setup
    Given an environment at "apiRoot"
    And the resource "/esim-remote-management/v0.1alpha1/profile/oper"
    And the header "Content-Type" is set to "application/json"
    And the header "Authorization" is set to a valid access token
    And the header "x-correlator" complies with the schema at "#/components/schemas/XCorrelator"
    And the request body is set based on the OAS schema at "/components/schemas/BaseCmpReqProfileOperReq"
    ...
    And the request body property "$.config.subscriptionDetail.optType" is set to a valid operation type (1 Enable, 2 Disable, 3 Delete)

# Success scenarios

  @esim_remote_management_profileOperation_01_generic_success_scenario
  Scenario: Common validations for any success scenario
    Given all request body properties are set to valid values
    When the request "profileOperation" is sent
    Then the response status code is 200
    And the response header "Content-Type" is "application/json"
    And the response header "x-correlator" has same value as the request header "x-correlator"
    And the response body complies with the OAS schema at "/components/schemas/BaseCmpRespProfileOperResp"

# Error scenarios

  @esim_remote_management_profileOperation_02_invalid_argument_scenario
  ...
  @esim_remote_management_profileOperation_03_out_of_range_scenario
  Scenario: Error response where request body parameters are out of range
    Given a request body property is out of range, for example an invalid optType value
    When the request "profileOperation" is sent
    Then the response status code is 400
    And the response header "Content-Type" is "application/json"
    And the response header "x-correlator" has same value as the request header "x-correlator"
  ...
  @esim_remote_management_profileOperation_04_missing_authorization_scenario
  ...
  @esim_remote_management_profileOperation_05_missing_access_token_scope_scenario
  Scenario: Missing access token scope
    Given the header "Authorization" is set to an access token that does not include scope esim-remote-management:oper
    When the request "profileOperation" is sent
  ...
  @esim_remote_management_profileOperation_06_not_found_scenario
  Scenario: Not found
